...
installAndLoad <- function(pkg)
{
    ...
    libdir <- file.path(tempfile(), "lib")
    if (!dir.create(libdir, recursive = TRUE, showWarnings=FALSE))
        stop("'dir.create' failed")
    stderr <- file.path(tempfile(), "install.stderr")
    if (!file.create(stderr, recursive = TRUE))

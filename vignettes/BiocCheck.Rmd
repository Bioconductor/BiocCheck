...
* Checks that it contains vignette sources (*.Rmd, *.Rnw, *.Rrst)
  (`REQUIRED`)
...
* Checks that a `biocViews` field is present in the DESCRIPTION file.
  (`RECOMMENDED`).
* Checks for valid views and displays invalid ones. (`RECOMMENDED`)
* Checks that all views come from the same parent (one of Software,
  AnnotationData, ExperimentData).
...
that every package meets the requirements.
...
* Checks to make sure the DESCRIPTION file has either a `Maintainer`
  ...
  `Maintainer field`, which consists of (the following are all `REQUIRED`):
...
asks you to `CONSIDER` adding more runnable examples.
...
Note that some text editors do not wrap lines automatically, requiring

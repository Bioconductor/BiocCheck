...
We recommend that `vapply()` be used instead of `sapply()`.

We recommend that `seq_len()` or `seq_along` be used instead of `1:...`. This is
because the case `1:0` comes out to equal `1:0` which may be an unexpected or
unwanted result. In addition, using `seq_len` or `seq_along` provides a slight
